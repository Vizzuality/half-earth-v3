...
const GridLayer = ({ view, setGridCellData, setGridCellGeometry }) => {
  ...
  useEffect(() => {
    if (viewExtent && gridLayer && gridCellGraphic) {
      ...
      gridLayer.queryFeatures(containedQueryObject).then(function (results) {
  ...
  useEffect(() => {
    if (aggregatedCells && !aggregatedCells.length > 0) {
      ...
      gridLayer.queryFeatures(centerCellQueryObject).then(function (results) {
        const { features } = results;
  ...
  const manageCellStoreAndGeomCreation = async (
    features,
    cellsIDsArray,
    type
  ) => {
