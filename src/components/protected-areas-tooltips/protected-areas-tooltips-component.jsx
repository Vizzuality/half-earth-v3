...
const ProtectedAreasTooltips = ({ view, isLandscapeMode, activeLayers }) => {
  ...
  const isProtectedAreasRendered =
    activeLayers &&
    activeLayers.find((l) => l.title === PROTECTED_AREAS_VECTOR_TILE_LAYER);
  const isCommunityAreasRendered =
    activeLayers &&
    activeLayers.find((l) => l.title === COMMUNITY_AREAS_VECTOR_TILE_LAYER);
  ...
  const displayTooltip = (point, content, color) => {
    ...
    view.popup.open({
      location: point,
      content: `
        <p class="tooltip-container">${tooltipDot(color)}${areaNameTooltip(
        content.NAME
      )}</p>${areaSizeTooltip(content.REP_AREA.toFixed(2))}`,
    });
  };
  return null;
};
