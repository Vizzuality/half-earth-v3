...
function AnalyzeAreasContainer(props) {
  ...
  const handleLayerToggle = (newSelectedOption) => {
    ...
    const getLayersToToggle = () => {
      ...
      // Future places layer will be activated if we select it at some point and
      // never toggled unless we do it from the protection checkbox
      const futureLayerIsActive = activeLayers.some((l) => {
        return (
          l.title === HALF_EARTH_FUTURE_TILE_LAYER
        );
      });
