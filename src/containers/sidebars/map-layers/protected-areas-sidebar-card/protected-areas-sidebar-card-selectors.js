...
const getComputedWDPALayers = createSelector(selectLangUrlState, (locale) =>
  getWDPALayers()
);
// locale is here to recompute the data when the language changes
...
export const getProtectionCountedActiveLayers = createSelector(
  [
    ...
    getComputedWDPALayers,
    getComputedConserveNextLayers,
  ],
  (activeLayers, WDPALayers, conserveNextLayers) => {
    ...
    const allLayers = Object.values(protectedLayers).map(
      (layer) => layer.value
    );
    return activeLayers
      .map((l) => l.title)
