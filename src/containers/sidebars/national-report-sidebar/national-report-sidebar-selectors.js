...
const selectCountryDataLoading = ({ countryData }) =>
  (countryData && countryData.loading) || null;
...
const getPriorityAreasSentence = createSelector(
  [getCountryData, getHasPriority, selectLangUrlState, getCountryName],
  ...
  Further expansion of `)}${countryName}${t(
          "'s"
        )}${t(` protected areas will nonetheless promote resilience towards global biodiversity loss,
  and can contribute to creating a global conservation network with more equity between countries.`)}`;
  }
);

const getSpeciesProtectionIndex = createSelector(
  getCountryData,
  (countryData) => {
    if (!countryData) return null;
...
export const getCurrentProtection = createSelector(
  getCountryData,
  (countryData) => {
    if (!countryData) return null;
    return Math.round(
      parseFloat(countryData[COUNTRY_ATTRIBUTES.prop_protected_ter])
    );
  }
);

export const getCurrentMarineProtection = createSelector(
  getCountryData,
  (countryData) => {
    if (!countryData) return null;
...
export const getProtectionNeeded = createSelector(
  getCountryData,
  (countryData) => {
    if (!countryData) return null;
    return Math.round(
      parseFloat(countryData[COUNTRY_ATTRIBUTES.protection_needed_ter])
    );
  }
);

export const getMarineProtectionNeeded = createSelector(
  getCountryData,
  (countryData) => {
    if (!countryData) return null;
...
const getNumberOfEndemicVertebrates = createSelector(
  getCountryData,
  (countryData) => {
    if (!countryData) return null;
...
const getHighlightedSpeciesSentence = createSelector(
  ...
  These species are either endemic to `)}${
      countryNames[countryData.NAME_0] || countryData.NAME_0
    } ${t('or have small range sizes.')}`;
  }
);

const getHighlightedSpeciesRandomNumber = createSelector(
  getCountryData,
  (countryData) => {
    if (!countryData) return null;
...
const getEndemicSpeciesSentence = createSelector(
  [getNumberOfEndemicVertebrates, selectLangUrlState],
  // eslint-disable-next-line no-unused-vars
  (endemicVertebrates, locale) => {
    if (!endemicVertebrates) return null;
    return endemicVertebrates === '1'
      ? `${endemicVertebrates} ${t('is')}`
      : `${endemicVertebrates} ${t('are')}`;
  }
);

const getTaxa = (taxa) =>
  createSelector(getCountryData, (countryData) => {
    if (!countryData) return null;
    return countryData[taxa];
  });
const getEndemicSpecies = (taxa) =>
  createSelector(getCountryData, (countryData) => {
    if (!countryData) return null;
...
const getSpeciesChartData = createSelector(getCountryData, (countryData) => {
  ...
  const chartData = [
    {
      name: 'endemic amphibians',
      value: endemic_amphibians,
      color: SPECIES_COLOR.amphibians,
      explodedSlice: true,
    },
    {
      name: 'amphibians',
      value: amphibians - endemic_amphibians,
      color: SPECIES_COLOR.amphibians,
      explodedSlice: false,
    },
    {
      name: 'endemic birds',
      value: endemic_birds,
      color: SPECIES_COLOR.birds,
      explodedSlice: true,
    },
    {
      name: 'birds',
      value: birds - endemic_birds,
      color: SPECIES_COLOR.birds,
      explodedSlice: false,
    },
    {
      name: 'endemic mammals',
      value: endemic_mammals,
      color: SPECIES_COLOR.mammals,
      explodedSlice: true,
    },
    {
      name: 'mammals',
      value: mammals - endemic_mammals,
      color: SPECIES_COLOR.mammals,
      explodedSlice: false,
    },
    {
      name: 'endemic mammals mar',
      value: endemic_mammals_mar,
      color: SPECIES_COLOR.mammals_mar,
      explodedSlice: true,
    },
    {
      name: 'mammals mar',
      value: mammals_mar - endemic_mammals_mar,
      color: SPECIES_COLOR.mammals_mar,
      explodedSlice: false,
    },
    {
      name: 'endemic fishes mar',
      value: endemic_fishes_mar,
      color: SPECIES_COLOR.fishes,
      explodedSlice: true,
    },
    {
      name: 'fishes mar',
      value: fishes_mar - endemic_fishes_mar,
      color: SPECIES_COLOR.fishes,
      explodedSlice: false,
    },
    {
      name: 'endemic reptiles',
      value: endemic_reptiles,
      color: SPECIES_COLOR.reptiles,
      explodedSlice: true,
    },
    {
      name: 'reptiles',
      value: reptiles - endemic_reptiles,
      color: SPECIES_COLOR.reptiles,
      explodedSlice: false,
    },
  ];
